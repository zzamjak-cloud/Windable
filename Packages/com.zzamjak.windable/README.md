...
```
https://github.com/zzamjak-cloud/Windable.git?path=/Packages/com.zzamjak.windable#v1.0.0
```
...
{
  "dependencies": {
    "com.zzamjak.windable": "https://github.com/zzamjak-cloud/Windable.git?path=/Packages/com.zzamjak.windable#v1.0.0"
  }
}
```
...
```
com.zzamjak.windable/
├── Runtime/
│   ├── Windable.cs               # 메인 컴포넌트
│   └── Resources/
│       ├── Noise.png             # 기본 노이즈 텍스처
│       └── Shader/
│           └── CAT_Windable.shader  # 바람 효과 셰이더
└── Editor/
    └── WindableEditor.cs         # 커스텀 인스펙터
```
...
## 소프트 마스크 대응 셰이더 (패키지 미포함)

이 패키지는 외부 의존성이 없도록 기본 셰이더만 포함합니다.
SoftMaskLight / mob-sakai SoftMask의 **소프트(알파) 마스킹**까지 적용하려면 아래 Hidden 변형 셰이더가 필요하며, 변형 셰이더는 마스크 시스템 소유 프로젝트(SoftMaskLight 플러그인의 `Shader/Hidden/`)에서 관리합니다.
...
- `SoftMaskLightInstaller.cs`의 `HiddenShaderNames` 배열에 두 변형 모두 등록 후 `Tools > SoftMaskLight > Refresh Settings` 실행 시 빌드에 자동 포함됩니다.
- 변형 셰이더 없이도 Unity 기본 Mask(Stencil), RectMask2D 마스킹은 정상 동작합니다.
